...
import { Send, CheckCircle, Mail, User, BookOpen, AlertCircle, Phone } from 'lucide-react';
...
export default function ContactForm({ prefilledProduct, prefilledBrand, onClearPrefill }: ContactFormProps) {
  ...
  const validate = () => {
    ...
    } else if (!/^\+?[0-9\s\-()]{10,20}$/.test(phone.replace(/\s+/g, ''))) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (sheetsUrl) {
      try {
        // Submit using URLSearchParams to avoid preflight CORS prechecks in Google Apps Script
        ...
        setErrors({ message: 'Network error. Please try again or call us directly.' });
        setIsSubmitting(false);
      }
    } else {
      // Fallback for local simulation when URL is not configured
      console.warn('Google Sheets URL not configured. Simulating successful submission.');
      setTimeout(() => {
        setIsSubmitting(false);
        setIsSuccess(true);
